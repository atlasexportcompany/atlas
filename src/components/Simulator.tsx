        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-800">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  />
                  <p className="text-xs text-slate-500 mt-1">Ej: 250 para un fardo premium.</p>
                </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-800 text-white rounded-xl focus:ring-2 focus:ring-rose-500 focus:border-rose-500 outline-none transition-all"
                  />
                  <p className="text-xs text-slate-500 mt-1">Ej: 100 prendas por fardo.</p>
                </div>
                <div>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl border border-slate-800">
              <h3 className="text-2xl font-bold mb-8 border-b border-slate-800 pb-4">Cotización Estimada</h3>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="pt-4 border-t border-slate-800 flex justify-between items-center">
                  ...
                  <span className="text-3xl font-bold text-rose-500">${totalCost.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                  <p className="text-sm text-slate-500 mb-1">Costo por Unidad/Fardo</p>
                  <p className="text-xl font-bold">${costPerUnit.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                  <p className="text-sm text-slate-500 mb-1">Costo por Artículo</p>
                  <p className="text-xl font-bold">${costPerItem.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
              </div>
            </div>

            <div className="bg-gradient-to-br from-rose-900/20 to-indigo-900/20 p-8 rounded-3xl shadow-sm border border-rose-500/20">
              <h3 className="text-xl font-bold text-rose-400 mb-6">Análisis de Rentabilidad</h3>
              
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div>
                  <p className="text-sm text-rose-300 mb-1">Ingresos Proyectados</p>
                  <p className="text-2xl font-bold text-rose-400">${totalRevenue.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
                <div>
                  <p className="text-sm text-rose-300 mb-1">Ganancia Neta</p>
                  <p className="text-2xl font-bold text-rose-400">${netProfit.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
                <div>
                  <p className="text-sm text-rose-300 mb-1">ROI Estimado</p>
                  <p className="text-2xl font-bold text-rose-400">{roi.toFixed(0)}%</p>
                </div>
              </div>

              <div className="mt-6 flex items-start text-sm text-rose-300 bg-rose-500/10 p-4 rounded-xl">
